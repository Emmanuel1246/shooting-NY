...
st.set_page_config(layout='wide') #para usar toda la pantalla del dashboard
st.markdown("<h1 style ='text-align: center; color: #1687CE;'>Histórico de disparos en Nueva York 🗽🔫 </h1>", unsafe_allow_html =True) #Titulo del dashboard

#st.write(df) #enviar al dashboard el dataframe

@st.cache(persist=True) #almacena la informacion en cache si parametro de url no cambia
def load_data(url):
    df=pd.read_csv(url)
...
#----------------------------------------------------------------------------------------------
c1,c2=st.columns((1,1)) #dividir en dos partes iguales el dashboard
c1.markdown("<h3 style ='text-align: center; color: black;'>¿Dónde han ocurrido disparos en NY?</h3>",unsafe_allow_html =True)


#crear mapa de donde han ocurrido disparos
year=c1.slider('Año en que ocurrió el suceso',int(df.year.min()),int(df.year.max())) #barra que cambia por el año
c1.map(df[df['year']==year][['latitude','longitude']])


#mapa a que horas ocurren disparos en NY
c2.markdown("<h3 style ='text-align: center; color: black;'>¿A que horas han ocurrido disparos en NY?</h3>",unsafe_allow_html =True)

hour=c2.slider('Hora en que ocurrió el suceso',int(df.hour.min()),int(df.hour.max())) #barra que cambia por la hora
df2=df[df['hour']==hour]
...
st.markdown("<h3 style ='text-align: center; color: black;'>cantidad de disparos por año/mes</h3>",unsafe_allow_html =True)
...
c4.markdown("<h3 style ='text-align: center; color: black;'>edad de los atacantes</h3>",unsafe_allow_html =True)
...
#  c4,c5,c6,c6=st.columns((1,1,1,1))

c5.markdown("<h3 style ='text-align: center; color: black;'>edad de las victimas</h3>",unsafe_allow_html =True)
...
c5.plotly_chart(fig)
##############################################
c6.markdown("<h3 style ='text-align: center; color: black;'>sexo del atacante</h3>",unsafe_allow_html =True)
df2=df.groupby(['perp_sex'])[['incident_key']].count().reset_index().sort_values('incident_key',ascending=False)

fig=px.pie(df2,values='incident_key',names='perp_sex',width=350,height=350)
# Editar gráfica
fig.update_layout(
        title_x=0.5,
        plot_bgcolor='rgba(0,0,0,0)',
...
##############################################
c7.markdown("<h3 style ='text-align: center; color: black;'>sexo de las victimas</h3>",unsafe_allow_html =True)
df2=df.groupby(['vic_sex'])[['incident_key']].count().reset_index().sort_values('incident_key',ascending=False)

fig=px.pie(df2,values='incident_key',names='vic_sex',width=350,height=350)
...
fig=px.bar(df2,x='year',y='incident_key',color='hour',barmode='group',width=1115,height=450)
